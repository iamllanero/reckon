...
# Get the DEBANK_ACCESSKEY environment variable
load_dotenv()
...
class HistoryList:
    ...
    def write(self):
        ...
        with open(
            f"{WALLETS_DIR}/{self.get_wallet_addr()}-{self.get_chain_id()}.json", "w"
        ) as f:
    ...
    def write_flat_csv(self):
        ...
        for i in range(self.get_size()):
            ...
            list_to_csv(
                flat_hl,
                f"{FLATTEN_DIR}/{self.get_wallet_addr()}-{self.get_chain_id()}.csv",
            )
    ...
    def get_token(self, id):
        ...
        if id in self.data["token_dict"]:
            ...
            token = {"name": "", "symbol": "", "is_verified": False}
        return token
    ...
    def get_project(self, id):
        ...
        if id in self.data["project_dict"]:
            ...
            return {"name": "", "chain": self.get_chain_id(), "site_url": ""}
    ...
    def get_history_entry_flat(self, index):
        ...
        num_rows = max(len(entry["receives"]), len(entry["sends"]), 1)
        ...
        for i in range(num_rows):
            ...
            row.extend([entry["id"], entry["other_addr"], get_tag(entry["other_addr"])])
            ...
            chain = project["chain"]
            row.extend([project["name"], chain, project["site_url"], entry["time_at"]])
            ...
            if len(entry["receives"]) > i:
                ...
                row.extend([token["symbol"], token["is_verified"]])
            ...
            if entry["tx"] is not None:
                ...
                if "usd_gas_fee" in entry["tx"]:
                    ...
                    row.extend([""])
                row.extend([entry["tx"]["value"], ":".join(entry["tx"]["params"])])
            else:
...
def fetch(url):
    ...
    headers = {"Accept": "application/json", "AccessKey": DEBANK_ACCESSKEY}
...
def create_history_list_url(id, chain_id, start_time=None):
    ...
    url = (
        "https://pro-openapi.debank.com/v1/user/history_list?"
        f"id={id}&"
        f"chain_id={chain_id}"
    )
